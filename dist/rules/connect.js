...
const rules = [
    {
        id: "1001",
        enable: true,
        valueDescription: "Site has been flagged as phishing by lux",
        valueDefine: {
            type: "boolean",
        },
        descriptions: {
            danger: "The website has been flagged as scam by lux",
        },
        defaultThreshold: {
            danger: true,
        },
        ...
        async getValue(ctx, apiService) {
            ...
            const { is_scam } = await apiService.getOriginIsScam(origin.url, "lux");
            return is_scam;
        },
    },
    {
    ...
    },
    {
        ...
        requires: ["origin"],
        async getValue(ctx) {
            return ctx.userData.originWhitelist.some((item) => caseInsensitiveCompare(item, ctx.origin.url));
        },
    },
    {
        id: "1070",
        enable: true,
        valueDescription: "Site has been verified by lux",
        descriptions: {
            safe: "The website has been verified by lux",
        },
        valueDefine: {
            type: "boolean",
        },
